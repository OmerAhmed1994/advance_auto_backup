...
class db_backup(models.Model):
    ...
    def schedule_backup(self):
        ...
        if conf_ids:
            for rec in conf_ids:
                db_list = rec.get_db_list(rec.host, rec.port)
                if rec.name in db_list:
                    ...
                    file_path, bkp_file, bkp_folder, bkp_folder_path = rec.local_backup()
                    
                    if rec.is_upload:
                        rec.google_drive_upload(file_path, bkp_file, bkp_folder, bkp_folder_path)
    ...
    def local_backup(self):
        ...
        bkp_file = '%s_%s.%s' % (self.name,date_today.strftime('%Y-%m-%d_%H_%M_%S'), self.backup_type)
        ...
        try:
            ...
            if self.backup_files:
                ...
                bkp_folder_path  = os.path.join(self.folder,bkp_folder) 
                with tempfile.TemporaryDirectory() as dump_dirf:
                    if os.path.exists(self.files_path):
                        shutil.copytree(self.files_path, os.path.join(dump_dirf, fpath))
            ...
            for f in os.listdir(dir):
                ...
                if self.backup_files:
                    ...
                    if fpath in fullpath:
                        timestamp = os.path.getmtime(os.path.join(dir, f)) 
                        createtime = datetime.datetime.fromtimestamp(timestamp)
                        now = datetime.datetime.now()
                        ...
                        if delta.days >= self.days_to_keep:
                            if os.path.isfile(fullpath) and ('.zip' in f):
                                _logger.info("Delete local out-of-date file: " + fullpath)
                                os.remove(fullpath)
        return file_path, bkp_file, bkp_folder, bkp_folder_path
    ...
    def sftp_backup(self):
        try:
            ...
            ipHost = self.sftp_host
            portHost = self.sftp_port
            usernameLogin = self.sftp_user
            ...
            if self.send_mail_sftp_fail:
                try:
                    ...
                    message = "Dear,\n\nThe backup for the server " + self.host + " (IP: " + self.sftp_host + ") failed.Please check the following details:\n\nIP address SFTP server: " + self.sftp_host + "\nUsername: " + self.sftp_user + "\nPassword: " + self.sftp_password + "\n\nError details: " + tools.ustr(
                        e) + "\n\nWith kind regards"
                    msg = ir_mail_server.build_email("advance_advance_auto_backup@" + self.name + ".com", [self.email_to_notify],
                                                    "Backup from " + self.host + "(" + self.sftp_host + ") failed",
                                                    message)
                    ir_mail_server.send_email(self._cr, self._uid, msg)
                except Exception:
                    pass

    def google_drive_upload(self, file_path, bkp_file, bkp_folder, bkp_folder_path):
        ...
        if self.is_upload:
            ...
            para = {
                "name": "%s" % (str(bkp_file)),
                "parents": ["%s" % (str(self.drive_folder_id))]
            }
            files = {
            ...
            if self.backup_files and bkp_folder and bkp_folder_path:
                para = {
                    "name": "%s" % (str(bkp_folder)),
                    "parents": ["%s" % (str(self.drive_folder_id))]
                }
                files = {
                    'data': ('metadata', json.dumps(para), 'application/json; charset=UTF-8'),
                    'file': open("%s" % (str(bkp_folder_path)), "rb")
                }
                r = requests.post(
                    "https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart",
                    headers=headers,
        ...
        if self.drive_autoremove and self.drive_to_remove:
            headers = {'Content-type': 'application/json', 'Accept': 'text/plain'}
            params = {
                'access_token': access_token,
                'q': "mimeType='application/%s'" % (self.backup_type),
